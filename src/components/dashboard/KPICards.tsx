...
export function KPICards({ stats }: KPICardsProps) {
  ...
  const kpis = [
    ...
    },
    {
      ...
      icon: ShieldAlert,
      color: stats.candidates_pending_gdpr > 0 ? "text-rose-600" : "text-gray-400",
      bg: stats.candidates_pending_gdpr > 0 ? "bg-rose-500/10" : "bg-gray-500/5",
    },
  ];
      ...
      {kpis.map((kpi, i) => (
        <Card key={i} className="border-none shadow-sm overflow-hidden group hover:shadow-md transition-all">
          <CardContent className="p-5">
            ...
            <h3 className="text-2xl font-black tracking-tight group-hover:scale-110 origin-left transition-transform">
              {kpi.value}
            </h3>
          </CardContent>
        </Card>
